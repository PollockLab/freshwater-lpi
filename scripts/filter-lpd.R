...
lpd_sp = lpd_filter.pts |> filter(sci_name %in% sp$sci_name)
pop_ids = lpd_sp$ID
saveRDS(lpd_sp, "data/filter-lpd.rds")
saveRDS(pop_ids, "data/lpd-popids.rds")
...
ggplot() +
  ...
  colorspace::scale_fill_discrete_qualitative(palette = "Dynamic") +
  labs(caption = paste0("Filtered LPI Database: ", nrow(lpd_sp),
